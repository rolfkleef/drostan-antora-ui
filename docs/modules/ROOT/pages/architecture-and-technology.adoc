...
:url-intellij: https://www.jetbrains.com/help/idea/connect-to-devcontainer.html

Antora requires a UI bundle to generate a documentation site.

This project contains the sources and build system to generate such a UI bundle.

== UI bundle contents

The UI bundle should contain {url-hbs}[Handlebars templates], CSS, JavaScript, images, and other assets.

This project contains the source files for such a UI bundle.
Those source files include those same Handlebars templates, images, fonts, and other assets.

The source files also include CSS, JavaScript, and Typescript code
that will be compiled and minified into the CSS and JavaScript for the UI bundle.

Source files can also be included via {url-npm}[npm packages].

== Sources and build system

The project is a {url-node}[Node.js] project
with {url-npm}[npm] as the package manager.

The {url-vite}[Vite] build tool compiles the source files into the UI bundle.
It also provides a development server to preview a design version of the UI locally.

Vite will combine several input files into a single output file.
This allows for a component-based organisation of source files.

The project uses {url-antora}[Antora] to generate a documentation site for this project itself.
This relies on Asciidoc sources for the content.
It also offers an end-to-end testing environment for the UI.

This project includes a {url-devcontainer}[Dev Containers] configuration
to provide a consistent development environment.
This works well in combination with {url-vscode}[Visual Studio Code],
and is also supported by {url-intellij}[IntelliJ IDEs].
...
regular dependencies that help create the UI bundle,
and development dependencies that help build the UI project.
These dependencies as well as build and test scripts are defined in the `package.json` file.

Vite and vite.config.js::
...
The [.term]*UI project* is comprised of the source files in the git repository, and provides the recipe and raw materials for creating an Antora UI bundle.
This is your development workspace.
+
The [.term]*UI bundle* is a distributable archive, and provides pre-compiled (interpreted, consolidated, and/or minimized) files that are ready to be used by Antora.
...
[source, treeview]
....
include::example$project-tree.source[]
....
...
<.> Local build of the UI bundle
<.> UI documentation
<.> GitHub action to generate the UI bundle
<.> Assets for the bundle
<.> Sources for templates, css, javascript
